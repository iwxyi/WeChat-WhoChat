...
- 主窗口信息架构：总览、聊天对象、分组、记忆、设置、诊断。
- 总览页接入真实运行态、当前聊天对象画像、近期聊天记录和记忆摘要，不再展示硬编码示例聊天。
...
- 联系人级第三方 AI 外发授权默认关闭，可在编辑资料中单独开启。
...
- 当前状态页已包含窗口、页面、联系人、采集、OCR、隐私和 AI 环节；后续继续增强可读性和排障信息密度。
...
- 诊断页可打开截图覆盖层式区域校准对话框，拖动导航、聊天列表、标题、聊天记录和输入区。
...
- 已加入明显居中时间锚点识别，拼接和入库会保存 OCR 解析出的 `message_time` 与 `time_source`。
...
- 未确认联系人、未知页面、低置信度 OCR 不写入长期记忆。
- 未确认联系人仍会阻断 AI 回复建议；疑似联系人消息可进入本地消息库，等待用户确认或合并。
...
- 已增加 PaddleOCR 自动采集稳定性保护：最低 8 秒间隔、运行中不排队、连续失败短期熔断，并写入 `capture_pipeline` / `ocr_worker` 诊断日志。
...
- API Key 优先写入 Windows Credential Manager。
- 配置文件不明文保存 API Key。
- 日志不记录 API Key。
...
- 结构化输出解析：候选回复、依据、风险。
- 手动回复保护、未知页面、未确认联系人会阻断可复制回复。
- 第三方 AI Provider 有 API Key 时，仍需要联系人级外发授权；未授权联系人只允许本地预览或直接阻断。
- 云端 AI 请求支持冷却时间、相同上下文去重和每日请求上限。
- AI Provider 连续失败会进入退避期，期间不再反复请求第三方接口。
- 云端请求前可弹出上下文预览，用户确认后才提交后台任务。
...
- 诊断页展示最近 AI 生成审计，便于排查为什么允许或阻断。
- AI Provider 诊断日志记录 endpoint 主机、耗时、状态和错误摘要，不记录 API Key、完整 prompt 或聊天原文。
- 诊断页已增加环境自检：Python、核心依赖、可选 OCR 依赖、数据目录写入能力和密钥后端。
- 诊断页已增加窗口匹配摘要：启用目标、相关候选窗口、进程/标题匹配原因，便于区分窗口识别失败与 OCR 失败。
- 诊断页日志支持按 debug/info/warning/error 过滤；复制日志和保存调试样本仍包含完整日志。
- 诊断复制和调试样本导出已接入最终脱敏层，即使未来日志误写入密钥、认证头、邮箱、链接、手机号或长数字，也会在导出边界拦截。
- Debug sample 会包含最后一次采集的结构化 layout、OCR boxes 和 parsed messages；开启截图样本保存后，可导出为 screenshot sample fixture。
...
- 已把 PaddleOCR 自动采集调为保守默认：重 OCR 间隔不低于 30 秒，子进程 25 秒超时，失败后熔断。
...
- 主路径端到端验证：窗口运行态、采集、OCR、入库、确认联系人、回复生成、悬浮窗复制和审计写入串联通过。
- 非聊天主路径验证：先生成可用建议后切到设置页，要求不新增聊天记录、不触发可用建议、悬浮窗旧按钮置灰。
- Offscreen UI smoke 会断言总览页使用真实联系人消息和记忆，避免回退到硬编码示例数据。
- 状态链路验证：未知页面、未确认联系人、OCR 运行/完成/失败/冷却、云端未授权和就绪状态都有明确环节状态。
- 微信窗口探针和截图探针。
...
- 环境诊断验证：诊断页、剪贴板 bundle 和调试样本都包含依赖、路径、窗口匹配摘要和密钥后端自检结果。
